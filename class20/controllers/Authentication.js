...
exports.Signup = async (req, res) => {
    try {
        ...
        const newUserData = await User.create({ firstName, lastName, email, password: hashPassword, otp: generatedotp, role });
...
exports.Login = async (req, res) => {
    try {
        ...
        if (await bcrypt.compare(password, userData.password)) {
            ...
            const token = await jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: "200s" });
...
exports.changePasswordFromProfile = async (req, res) => {
    try {
        ...
        // const{id}=req.id;//both are same
        // check karo ki email already to db me to nhi hai
        const userData = await User.findOne({ _id: id });
        console.log("userData", userData);

        // cureent password jo body se daloge use database ke andar jo password store hai us se compare karao
        ...
        if (await bcrypt.compare(currentPassword, userData.password)) {
            ...
            try {
                ...
                // upfdate password in db
                await User.findByIdAndUpdate({ _id: userData._id }, { password: hashPassword })
                SendEmail(userData.email, "Password Reset Suceessfully", userData.firstName, undefined, "resetPassword");
            ...
            return res.status(400).json({
                message: "You have entered wrong current password value",
                success: false,
            })
        }
